...
Page({
  ...
  search_keyword(e) {
    ...
    formSubmitjdzd() {
	    ...
	    wx.navigateTo({
        url: `hotel/jdzd?customer_lon=${data.longitude}&customer_lat=${data.latitude}&startDate=${inTime}&endDate=${outTime}&price=${data.priceId}&hotelStar=${data.starId}&citySelect=${data.citySelect}&address=${data.address}&cityData=${data.cityData}&search=${data.search_keyword}`
	    });
  	},
    closeTip() {
        ...
        wx.getStorage({
            ...
            fail: () => {
                if (date.getHours() >= 12) {
                    ...
                    outDate = '后天';
                    w = date.getDay() + 1;
                    starTime = formatMonth(Date.now() + 1000*60*60*24);
                    endTime = formatMonth(Date.now() + (1000*60*60*24) * 2);
                    // 传输数据
                    inTime = formatDate(Date.now() + 1000*60*60*24);
                    outTime = formatDate(Date.now() + (1000*60*60*24) * 2);
                    if (w == 7) {
                        start = '周日';
                    } else {
                        start = '周' + integer(w);
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      title: '秀豹酒店直订',
      path: '/pages/index',//这里填写首页的地址,一般为/pages/xxxx/xxx
